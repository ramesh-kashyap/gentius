...
class Register extends Controller
{
    ...
    // ✅ Check if that position under sponsor is already taken
        $existingUserInPosition = User::where('sponsor', $user->id)
            ->where('position', $request->position)
            ->exists();

        if ($existingUserInPosition) {
            return Redirect::back()->withErrors("Sponsor's {$request->position} position is already filled.")->withInput();
        }
